...
export const getAdminDashboardData = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const totalBooks = await prisma.physicalBook.count({
      where: { libraryId },
    });    const availableBooks = bookInventory.find((b: any) => b.status === 'AVAILABLE')?._count.status || 0;
    const borrowedBooks = bookInventory.find((b: any) => b.status === 'BORROWED')?._count.status || 0;    // Book categories
    ...
    const categoryIds = bookCategories.map((c: any) => c.categoryId);
    ...
    const categoriesWithCounts = bookCategories.map((bc: any) => {
      const category = categories.find((c: any) => c.id === bc.categoryId);
      return {
