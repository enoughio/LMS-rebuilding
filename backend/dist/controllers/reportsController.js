...
// ==========================================
export const getLibraryPerformanceReports = async (req, res) => {
    try {
        const { startDate, endDate, libraryId } = req.query;
        const start = startDate ? new Date(startDate) : new Date(new Date().setMonth(new Date().getMonth() - 1));
        const end = endDate ? new Date(endDate) : new Date();
        // Build where clause for library filtering
        const libraryFilter = libraryId && libraryId !== 'all' ? { id: libraryId } : {};
        ...
        const libraryPerformance = await Promise.all(libraries.map(async (library) => {
...
// ==========================================
export const getLibrariesForReports = async (_req, res) => {
    try {
        const libraries = await prisma.library.findMany({
            where: { status: 'APPROVED' },
            select: {
