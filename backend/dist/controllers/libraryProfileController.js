...
export const updateLibraryProfile = async (req, res) => {
    try {
        ...
        const { name, description, address, city, state, country, postalCode, email, phone, amenities, totalSeats, images, openingHours, imagesToDelete } = req.body;
        ...
        const updatedLibrary = await prisma.$transaction(async (tx) => {
            ...
            if (images && Array.isArray(images)) {
                ...
                const existingUrls = images.filter((img) => typeof img === 'string' && img.startsWith('http'));
