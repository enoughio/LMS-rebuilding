...
export const getLibrarySeats = async (req, res) => {
    try {
...
export const deleteSeat = async (req, res) => {
    try {
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            const activeBookings = seat.bookings.filter(booking => booking.status === 'CONFIRMED' || booking.status === 'PENDING');
    ...
    try {
        ...
        try {
            ...
            if (userEmail && userName) {
                const emailTemplate = getSeatBookingConfirmationTemplate({
                    userName,
                    userEmail,
...
const generateBookingBill = async (booking, payment) => {
...
export const cancelBooking = async (req, res) => {
    try {
        ...
        try {
            const user = booking.user || booking.guestUser;
            if (user?.email && user?.name) {
                const emailTemplate = getSeatBookingCancellationTemplate({
                    userName: user.name,
