...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${API_URL}/api/reports/users?${queryParams.toString()}`, {
      method: 'GET',
      headers: {
    ...
    if (!response.ok) {
      ...
      console.error('Failed to fetch user activity reports:', errorData.message);
      return NextResponse.json(
        { success: false, message: errorData.message || 'Failed to fetch user activity reports' },
