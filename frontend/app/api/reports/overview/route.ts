...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${API_URL}/api/reports/overview?${queryParams.toString()}`, {
      method: 'GET',
      headers: {
    ...
    if (!response.ok) {
      ...
      console.error('Failed to fetch reports overview:', errorData.message);
      return NextResponse.json(
        { success: false, message: errorData.message || 'Failed to fetch reports overview' },
