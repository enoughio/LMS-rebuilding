...
export async function GET() {
  try {
    ...
    const response = await fetch(`${API_URL}/api/reports/libraries-list`, {
      method: 'GET',
      headers: {
    ...
    if (!response.ok) {
      ...
      console.error('Failed to fetch libraries list:', errorData.message);
      return NextResponse.json(
        { success: false, message: errorData.message || 'Failed to fetch libraries list' },
