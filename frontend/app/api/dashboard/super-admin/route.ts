...
export async function GET() {
  try {
    ...
    if (!response.ok) {
      ...
      console.error('Failed to fetch super admin data:', errorData.message);
      return NextResponse.json(
        { success: false, message: errorData.message || 'Failed to fetch super admin data' },
    ...
    return NextResponse.json(
      { success: false, error: 'Internal server error', message: 'Failed to fetch super admin data' },
      { status: 500 }
    );
  }
}