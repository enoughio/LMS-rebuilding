...
export default function PostDetail({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    const fetchPostDetail = async () => {
      ...
      try {
        ...
        setError('Failed to load the post. It may have been removed or you don\'t have permission to view it.');
      } finally {
        setLoading(false);
      }
    };
  ...
  const fetchComments = async (page: number) => {
    ...
    try {
      ...
      setCommentPagination(result.meta);
  ...
  const handleSubmitComment = async () => {
    ...
    try {
      await forumApi.addComment(params.id, comment);
      setComment('');
